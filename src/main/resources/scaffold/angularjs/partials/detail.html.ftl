<form id="new-${entityName}" name="new${entityName}" class="form-horizontal">
    ...
    <div class="control-group">
        ...
        <div class="controls">
            ...
            <select id="${property.name}" name="${property.name}" ng-model="${entityName?uncap_first}.${property.name}" ng-options="${property.name?substring(0, 1)} as ${property.name?substring(0, 1)}.id for ${property.name?substring(0, 1)} in ${property.name}List">
                <option value="">Choose a ${property.name?cap_first}</option>
            </select>
            <#else>
            <input id="${property.name}" name="${property.name}" <#if property.type == "number">type="number"<#elseif (property["datetime-type"]!"") == "date">type="date"<#elseif (property["datetime-type"]!"") == "time">type="time"<#elseif (property["datetime-type"]!"") == "both">type="datetime"<#else>type="text"</#if> <#if property.required!"false" == "true">required</#if> ng-model="${entityName?uncap_first}.${property.name}" placeholder="Enter the ${entityName} ${property.name}"></input>
            </#if>
        </div>
        </#if>
    </div>
